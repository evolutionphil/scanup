...
export const useAuthStore = create<AuthState>((set, get) => ({
    ...
    if (Platform.OS !== 'web') {
      setTimeout(async () => {
        try {
          ...
          if (pushToken) {
            await savePushTokenToBackend(pushToken);
            console.log('[Auth] Push token registered after register');
          }
        } catch (err) {
          console.log('[Auth] Push token registration failed:', err);
        }
    ...
    if (Platform.OS !== 'web') {
      setTimeout(async () => {
        try {
          ...
          if (pushToken) {
            await savePushTokenToBackend(pushToken);
            console.log('[Auth] Push token registered after Google login');
          }
        } catch (err) {
          console.log('[Auth] Push token registration failed:', err);
        }
        ...
        try {
          const { restore, verifySubscriptionStatus } = usePurchaseStore.getState();
          console.log('[Auth] Restoring purchases after Google login...');
          await restore();
          await verifySubscriptionStatus();
          console.log('[Auth] Purchase restoration complete');
        } catch (err) {
  ...
  googleLoginNative: async (idToken: string, googleUser: any) => {
    ...
    // ⭐ CRITICAL: Sync premium status from backend for ALL platforms (including web)
    const userIsPremium = data.user?.is_premium || data.user?.subscription_type === 'premium' || data.user?.subscription_type === 'trial';
    console.log('[Auth] User premium status from backend (Google native login):', userIsPremium);
    usePurchaseStore.getState().syncWithUser(userIsPremium);
    ...
    if (Platform.OS !== 'web') {
      setTimeout(async () => {
        try {
          ...
          if (pushToken) {
            await savePushTokenToBackend(pushToken);
            console.log('[Auth] Push token registered after Google native login');
          }
        } catch (err) {
          console.log('[Auth] Push token registration failed:', err);
        }
        ...
        try {
          const { restore, verifySubscriptionStatus } = usePurchaseStore.getState();
          console.log('[Auth] Restoring purchases after Google native login...');
          await restore();
          await verifySubscriptionStatus();
          console.log('[Auth] Purchase restoration complete');
        } catch (err) {
  ...
  appleLogin: async (data: any) => {
    ...
    // ⭐ CRITICAL: Sync premium status from backend for ALL platforms (including web)
    const userIsPremium = data.user?.is_premium || data.user?.subscription_type === 'premium' || data.user?.subscription_type === 'trial';
    console.log('[Auth] User premium status from backend (Apple login):', userIsPremium);
    usePurchaseStore.getState().syncWithUser(userIsPremium);
  },
